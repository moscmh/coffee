...
espresso_coffee = pd.get_dummies(espresso_coffee, columns=['Origin'])
filter_coffee = pd.get_dummies(filter_coffee, columns=['Origin'])

origins = ['Ethiopia', 'Colombia', 'Brazil', 'Guatemala', 'Costa Rica', 'Kenya', 'Honduras', 'Mexico', 'Peru', 'Panama', 'Indonesia', 'Other']
for o in origins:
    if 'Origin_' + o not in espresso_coffee.columns:
        espresso_coffee['Origin_' + o] = False
    if 'Origin_' + o not in filter_coffee.columns:
        filter_coffee['Origin_' + o] = False
